...
def test_mc_param_no_boundaries():
    ...
    # prepare
    optimal_control = oc.OptimalControl(fields=['a', 'b', 'c'], beta=1)
    KM, p = optimal_control.keymap, \
            optimal_control.zero_parameters(2)
    ...
    optimal_control.rand = RandMock([
        [0, 0], # first parameters
        [0, 0], # decrease
    ])
    p1 = optimal_control.mc_param(p)
    ...
    p, p1 = p1, optimal_control.mc_param(p1)
    ...
    optimal_control.rand = RandMock([[0.0, 0.3411], [.501, .499]])
    p, p1 = p1, optimal_control.mc_param(p1)
    ...
    optimal_control.rand = RandMock([[0.8, 0.1], [.1, .89]])
    p, p1 = p1, optimal_control.mc_param(p1)
...
def test_mc_param_boundaries():
    ...
    # prepare
    optimal_control = oc.OptimalControl(fields=['a', 'b'], beta=1)
    KM, p = optimal_control.keymap, \
            optimal_control.zero_parameters(2)
    ...
    optimal_control.rand = RandMock([[PA, PB], [INC, INC]])
    p1 = optimal_control.mc_param(p)
...
def test_mc_param_forbidden():
    ...
    optimal_control.rand = RandMock([[PA, PB], [DEC, DEC]])
    p, p1 = p1, optimal_control.mc_param(p1, p['p'])
    assert_allclose([1.2, 2.0], p1[:, KM['a']]['p'], **QOP.TEST_TOLS)
    assert_allclose([0.0, -1], p1[:, KM['b']]['p'], **QOP.TEST_TOLS)


@pytest.mark.parametrize("beta, random_numbers, optimize, x0, x1, xe", [
    [0, [0], oc.Optimize.MAXIMIZE, [0, 0, 0], [0, -0.001, 0.001], [0, 0, 0.001]],
    [0, [0], oc.Optimize.MINIMIZE, [0, 0, 0], [0, -0.001, 0.001], [0, -0.001, 0]],
    [
        1,
        np.array([0.4999999, 0.5, 0.5000001]).reshape((3,)),
        oc.Optimize.MAXIMIZE,
        [0, 0,          0,          0],
    ...
    [
        3.0,
        np.array([0.5**3 - 0.001, 0.5**3, 0.5**3 + 0.001]).reshape((3,)),
        oc.Optimize.MAXIMIZE,
        [0, 0,          0,          0],
        [0, -np.log(2), -np.log(2), -np.log(2)],
        [0, 0, 0, -np.log(2)]
...
def test_arg_discard(beta, optimize, random_numbers, x0, x1, xe):
    """
    ...
    didx     = oc.arg_discard(optimize.value * (x0 - x1), beta, RandMock([random_numbers]))
    xs       = np.array(x1, copy=True)
    xs[didx] = x0[didx]
    assert_array_equal(xe, xs)

    # test object api ----------------------------
    optimal_control      = oc.OptimalControl(fields=['a', 'b'], beta=beta, optimize=optimize)
    optimal_control.rand = RandMock([random_numbers])

    didx     = optimal_control.arg_discard(x0, x1)
    xs       = np.array(x1, copy=True)
    xs[didx] = x0[didx]
    assert_array_equal(xe, xs)
...
def test_arg_eps_bad_shape():
    ...
    """
    try:
        oc.arg_eps(np.zeros((2,3)), np.zeros((2,4)))
...
def test_arg_eps():
    ...
    """
    KM, p = oc.zero_parameters(4, ['a', 'b'])

    _ = 0.0
    p[:, KM['a']] = [(0.1, _, _, _, 0.1),   # will just increase by constant 0.1
                     (0.1, _, _, _, 0.15),  # hopping around
                     (0.0, _, _, _, 0.0),   # must be equal
                     (0.0, _, _, _, 0.1)]   # good, but the last will be bad
    p[:, KM['b']] = [(1.0, _, _, _, 0.1),   # will converge
                     (2.0, _, _, _, 0.3),   # + 0.1, mean = 2.3
                     (3.0, _, _, _, 0.3),   # + 0.1, mean = 3.3, eps adapative (1)
                     (4.0, _, _, _, 0.3)]   # + 0.1, mean = 3.3, eps adaptive (2)
    P = np.array(p.reshape(1, *p.shape), copy=True)
    p[:, KM['a']] = [(0.2, _, _, _, 0.1),
                     (-0.1, _, _, _, 0.15),
                     (0.001, _, _, _, 0.0),
                     (0.05, _, _, _, 0.1)]
    p[:, KM['b']] = [(1.2, _, _, _, 0.1),
                     (2.1, _, _, _, 0.3),
                     (3.1, _, _, _, 0.2),
                     (4.0, _, _, _, 0.1)]
    P = np.concatenate((P, p.reshape(1, *p.shape)))
    p[:, KM['a']] = [(0.3, _, _, _, 0.1),
                     (0.1, _, _, _, 0.15),
                     (0.1, _, _, _, 0.0),
                     (0.0, _, _, _, 0.1)]
    p[:, KM['b']] = [(1.3, _, _, _, 0.1),
                     (2.2, _, _, _, 0.3),
                     (3.2, _, _, _, 0.1),
                     (4.0, _, _, _, 0.1)]
    P = np.concatenate((P, p.reshape(1, *p.shape)))
    p[:, KM['a']] = [(0.4, _, _, _, 0.1),
                     (-0.1, _, _, _, 0.15),
                     (0.11, _, _, _, 0.0),
                     (0.05, _, _, _, 0.1)]
    p[:, KM['b']] = [(1.35, _, _, _, 0.1),
                     (2.3, _, _, _, 0.3),
                     (3.3, _, _, _, 0.0),
                     (4.0, _, _, _, 0.0)]
    P = np.concatenate((P, p.reshape(1, *p.shape)))
    p[:, KM['a']] = [(0.5, _, _, _, 0.1),
                     (0.1, _, _, _, 0.15),
                     (0.111, _, _, _, 0.0),
                     (0.0, _, _, _, 0.1)]
    p[:, KM['b']] = [(1.40, _, _, _, 0.1),
                     (2.4, _, _, _, 0.3),
                     (3.4, _, _, _, 0.1),
                     (4.0, _, _, _, np.inf)]
    P = np.concatenate((P, p.reshape(1, *p.shape)))
    p[:, KM['a']] = [(0.6, _, _, _, 0.1),
                     (-0.1, _, _, _, 0.15),
                     (0.1111, _, _, _, 0.0),
                     (0.05, _, _, _, 0.1)]
    p[:, KM['b']] = [(1.42, _, _, _, 0.1),
                     (2.5, _, _, _, 0.3),
                     (3.5, _, _, _, 0.2),
                     (4.0, _, _, _, 0.2)]
    P = np.concatenate((P, p.reshape(1, *p.shape)))
    p[:, KM['a']] = [(0.7, _, _, _, 0.1),
                     (0.1, _, _, _, 0.15),
                     (0.1111, _, _, _, 0.0),
                     (0.151, _, _, _, 0.1)]
    p[:, KM['b']] = [(1.41, _, _, _, 0.1),
                     (2.6, _, _, _, 0.3),
                     (3.6, _, _, _, 0.3),
                     (4.0, _, _, _, np.inf)] # infinite should be allowed too
    P = np.concatenate((P, p.reshape(1, *p.shape)))
